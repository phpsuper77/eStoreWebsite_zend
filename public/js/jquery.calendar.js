	...
	$.fn.slider = function(options) {
		...
		var _draw = function() {
			...
			if (settings.showProgress) {
				$this.find('.jquery-slider-page').removeClass(
						'jquery-slider-page-current');
				$this
						.find(
								'.jquery-slider-page:eq('
										+ (_this
												.nextAll('.jquery-slider-element').length)
										+ ')').addClass(
								'jquery-slider-page-current');
			}
		...
		var _init = function() {
			...
			$this.addClass('jquery-slider').width(settings.width).height(
					settings.height);
			$this
					.children()
					.each(
							function() {
								...
								positionEls
										.prepend($(
												'<span class="jquery-slider-page"></span>')
												.bind(
														'click',
														function() {
															if ($this
																	.hasClass('jquery-slider-sliding'))
																return;
															_last = _this;
															_this = $tmp;
															_draw();
			...
			if (settings.showControls) {
				var controlPrev = $(
						'<span class="jquery-slider-control jquery-slider-control-prev"></span>')
						.bind('click', function() {
							_prev();
						});
				var controlNext = $(
						'<span class="jquery-slider-control jquery-slider-control-next"></span>')
						.bind('click', function() {
							_next();
						});
			...
			if (settings.randomize) {
				_this = $this
						.children('.jquery-slider-element')
						.eq(
								parseInt($this
										.children('.jquery-slider-element').length
										* Math.random()));
			}
			_cycle();
		};
